...
function getTrendChartOption(trend: TrendItem[]) {
  return {
    ...
    xAxis: { type: 'category', data: trend.map(t => textValue(t['日期']).slice(5)), axisLabel: { color: '#64748b' } },
    yAxis: [
      { type: 'value', min: 0, axisLabel: { formatter: '{value}%' }, splitLine: { lineStyle: { color: '#1e293b' } } },
      { type: 'value', splitLine: { show: false } },
    ],
    series: [
      { name: '通过率(%)', type: 'line', data: trend.map(t => numberValue(t['通过率%'])), smooth: true, symbol: 'circle', symbolSize: 6, lineStyle: { color: '#06b6d4', width: 2 }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(6,182,212,0.15)' }, { offset: 1, color: 'rgba(6,182,212,0)' }] } } },
...
function getProvinceChartOption(data: Record<string, number | string>[]) {
  return {
    ...
    series: [{
      type: 'bar',
      data: data.map(p => ({ value: numberValue(p['通过率%']), itemStyle: { color: Number(numberValue(p['通过率%'])) < 50 ? '#ef4444' : Number(numberValue(p['通过率%'])) < 60 ? '#f59e0b' : '#06b6d4', borderRadius: [3, 3, 0, 0] } })),
      barWidth: '50%',
                ...
                {passRate.compare.length > 0 && (
                  <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-4">
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-3">
                      {(() => {
                        const ok = passRate.compare.filter((c) => String(c['评估']).includes('达标') || String(c['评估']).includes('优秀')).length;
                        const bad = passRate.compare.filter((c) => String(c['评估']).includes('不达标')).length;
                        return (
                          <>
