...
import type { PassRateReport, TrendItem, ProvincePassRate, CompareItem } from '@/services/api';
...
function getTrendChartOption(trend: TrendItem[]) {
  return {
    ...
    xAxis: { type: 'category', data: trend.map((t) => textValue(t['日期']).slice(5)), axisLine: { lineStyle: { color: '#334155' } }, axisLabel: { color: '#64748b' } },
    yAxis: [
      { type: 'value', min: 0, axisLabel: { formatter: '{value}%' }, splitLine: { lineStyle: { color: '#1e293b' } } },
      { type: 'value', splitLine: { show: false } },
    ],
    series: [
      { name: '通过率(%)', type: 'line', data: trend.map((t) => numberValue(t['通过率%'])), smooth: true, symbol: 'circle', symbolSize: 6, lineStyle: { color: '#06b6d4', width: 2 }, itemStyle: { color: '#06b6d4' }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(6,182,212,0.15)' }, { offset: 1, color: 'rgba(6,182,212,0)' }] } } },
      { name: '申请数', type: 'bar', yAxisIndex: 1, data: trend.map((t) => numberValue(t['申请数'])), itemStyle: { color: '#8b5cf6', borderRadius: [3, 3, 0, 0] }, barWidth: '30%' },
    ],
  };
}

function getProvinceChartOption(data: ProvincePassRate[]) {
  return {
    ...darkChartTheme,
    tooltip: { trigger: 'axis', backgroundColor: '#1e293b', borderColor: '#334155', textStyle: { color: '#e2e8f0' } },
    xAxis: { type: 'category', data: data.map((p) => textValue(p['省份']).replace(/省|回族自治区|壮族自治区|维吾尔自治区|自治区/g, '')), axisLabel: { rotate: 45 } },
    yAxis: { type: 'value', axisLabel: { formatter: '{value}%' }, splitLine: { lineStyle: { color: '#1e293b' } } },
    grid: { bottom: 80 },
    series: [{
      type: 'bar',
      data: data.map((p) => ({ value: numberValue(p['通过率%']), itemStyle: { color: numberValue(p['通过率%']) < 50 ? '#ef4444' : numberValue(p['通过率%']) < 60 ? '#f59e0b' : '#06b6d4', borderRadius: [3, 3, 0, 0] } })),
      barWidth: '50%',
...
function getCompareChartOption(compare: CompareItem[]) {
  return {
    ...darkChartTheme,
    tooltip: { trigger: 'axis', backgroundColor: '#1e293b', borderColor: '#334155', textStyle: { color: '#e2e8f0' } },
    legend: { data: ['实际通过率(%)', '标准通过率(%)'], top: 0 },
    xAxis: { type: 'category', data: compare.map((p) => textValue(p['省份']).replace(/省|回族自治区|壮族自治区|维吾尔自治区|自治区/g, '')), axisLabel: { rotate: 45 } },
    yAxis: { type: 'value', axisLabel: { formatter: '{value}%' }, splitLine: { lineStyle: { color: '#1e293b' } } },
    grid: { bottom: 80 },
    series: [
      { name: '实际通过率(%)', type: 'bar', data: compare.map((p) => ({ value: numberValue(p['通过率%']), itemStyle: { color: '#06b6d4', borderRadius: [3, 3, 0, 0] } })), barWidth: '30%' },
      { name: '标准通过率(%)', type: 'bar', data: compare.map((p) => ({ value: numberValue(p['标准通过率%']), itemStyle: { color: '#f59e0b', borderRadius: [3, 3, 0, 0] } })), barWidth: '30%' },
    ],
  };
}
              ...
              <AnimatedSection delay={0.2}>
                <div className="bg-[#1e293b] border border-[#334155] rounded-xl p-4">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3">
                    {(() => {
                      const ok = data.compare.filter((c) => String(c['评估']).includes('达标') || String(c['评估']).includes('优秀')).length;
                      const bad = data.compare.filter((c) => String(c['评估']).includes('不达标')).length;
                      return (
                        <>
